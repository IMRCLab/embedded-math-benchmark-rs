use crate::tasks::{
    LeeController as LeeControllerTask, MatInverse3x3 as MatInverse3x3Task, MatMul3x3 as MatMul3x3Task, QuatMul as QuatMulTask,
...
impl RawTaskImplementation<LeeControllerTask> for LeeControllerLogic {
    type PreparedInput = (
        nalgebra::Vector3<f32>, nalgebra::Vector3<f32>, nalgebra::UnitQuaternion<f32>, nalgebra::Vector3<f32>, // state
        nalgebra::Vector3<f32>, nalgebra::Vector3<f32>, nalgebra::Vector3<f32>, f32, f32, // cmd
        f32 // mass
    );
    ...
    fn prepare(
        ...
        (
            ...
            uq,
            nalgebra::Vector3::new(input.angular_velocity[0], input.angular_velocity[1], input.angular_velocity[2]),
            nalgebra::Vector3::new(input.setpoint_position[0], input.setpoint_position[1], input.setpoint_position[2]),
            nalgebra::Vector3::new(input.setpoint_velocity[0], input.setpoint_velocity[1], input.setpoint_velocity[2]),
            nalgebra::Vector3::new(input.setpoint_acceleration[0], input.setpoint_acceleration[1], input.setpoint_acceleration[2]),
            input.setpoint_yaw,
