# Archive: GitLab HIL CI setup

> **Note.** Superseded 2026-09 by the move to GitHub Actions. Kept for reference in case the lab
> ever reverts, or as a record of the GitLab-runner provisioning steps. The live doc is
> [hil-setup.md](../hil-setup.md). The actual `.gitlab-ci.yml` and `.gitlab/ci/*.yml` job
> definitions this doc describes were removed from the tree here; last present at commit
> `c5f447200fefc4b4f73e23dc21195c6d650eba5d` (`git show
> c5f447200fefc4b4f73e23dc21195c6d650eba5d:.gitlab-ci.yml`, etc.).
...
`espup` Xtensa toolchain. `Dockerfile.ci` layers those on top and `.gitlab/ci/image.yml` rebuilds
it when the Dockerfile or the job changes, using kaniko so it can build unprivileged. Targets are
baked into the image, so a new one must be added there too.
...
roughly halves a run job. Set `PROBE_SPEED` (kHz) per target in `.gitlab/ci/run.yml`: 4000 is the
fastest step the J-Link accepts, and the Debug Probes take 10000. Leave it unset for esp32s3,
whose USB JTAG is a bridge with no settable clock.
...
See `tools/rp2350-rescue/README.md`.
