...
#let profile-def(name, body) = block(width: 100%)[
  #text(weight: 800, size: 19pt)[#raw(name)]
  #h(6pt)
  #text(size: 19pt)[#body]
]
...
#let infrastructure-panel() = block(width: 100%)[
  ...
  #grid(
    ...
    profile-def("xlto", [clang compiles the C to LLVM bitcode, and one ThinLTO pass covers Rust and C, so C calls can be inlined.]),
