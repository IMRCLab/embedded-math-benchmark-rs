...
#let infrastructure-panel() = block(width: 100%)[
  ...
  #grid(
    ...
    stat-block("4 Rust libraries", [glam, nalgebra (linear algebra) #sym.dot.c micromath (fast approximations) #sym.dot.c libm (elementary and transcendental math for bare-metal no_std)]),
    stat-block("2 C libraries", [CMSIS-DSP (Arm's signal-processing library) #sym.dot.c cmath3d (the Crazyflie drone firmware's 3D math). Elementary and transcendental math functions come from newlib.]),
