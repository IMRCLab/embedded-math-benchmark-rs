...
#let infrastructure-panel() = block(width: 100%)[
  ...
  #grid(
    ...
    stat-block(
      "18 math primitives",
      [909 inputs baked into flash at compile time. Only the math call is timed, in on-chip clock cycles.],
    ),
    stat-block(
      "4 Rust libraries",
      [glam, nalgebra (linear algebra) #sym.dot.c micromath (fast approximations) #sym.dot.c libm (elementary and transcendental math for bare-metal no_std)],
    ),
    stat-block(
      "2 C libraries",
      [CMSIS-DSP (Arm's signal-processing library) #sym.dot.c cmath3d (the Crazyflie drone firmware's 3D math). Elementary and transcendental math functions come from newlib.],
    ),
    stat-block(
      "5 chips",
      [STM32F405, nRF52840 (Cortex-M4F) #sym.dot.c RP2350 (M33) #sym.dot.c RP2040 (M0+, no FPU) #sym.dot.c ESP32-S3 (Xtensa)],
    ),
  )
  #v(16pt)
  ...
  #text(
    size: 19pt,
  )[C code is always called from Rust through FFI, as when firmware wraps a C library. What the optimizer can see across that call decides the result.]
  #v(10pt)
  #grid(
    columns: (1fr, 1.25fr, 1.25fr, 1.2fr),
    column-gutter: 30pt,
